...
class KeyValueArgOptionDefinition(
  ...
  a.indexOf('=') match {
    ...
    case n: Int => action(a.take(n), a.drop(n + 1))
  }},
  false, true)
...
class KeyIntValueArgOptionDefinition(
  ...
  a.indexOf('=') match {
    case -1     => throw new IllegalArgumentException("Expected a key=value pair")
    case n: Int => action(a.take(n), a.drop(n + 1).toInt)
  }},
  false, true)
...
class KeyDoubleValueArgOptionDefinition(
  ...
  a.indexOf('=') match {
    case -1     => throw new IllegalArgumentException("Expected a key=value pair")
    case n: Int => action(a.take(n), a.drop(n + 1).toDouble)
  }},
  false, true)
...
case class OptionParser(
  ...
  def help(shortopt: String, longopt: String) =
    add(new FlagOptionDefinition(shortopt, longopt, "show this help message", {this.showUsage; exit}))
      ...
      matchingOption match {
        ...
        case Some(option) =>
          val argToPass = if (option.gobbleNextArgument) {
            ...
            arg.drop(("-" + option.shortopt + ":").length).toString
          } else if (option.keyValueArgument && arg.startsWith("--" + option.longopt + ":")) {
            arg.drop(("--" + option.longopt + ":").length).toString
          } else
            ""
